...
-노트북은 컴퓨터의 일종이다.
-휴대폰은 전화기의 일종이다.
이와같이 Is-A 관계가 성립하는 객체를 상속으로 표현하기에 접합하다.
...
class NotebookComputer extends Computer {
	...
	public NotebookComputer(String name, int initCharge) {
		/* 부모클래스의 생성자를 호출하기 위해 사용하는 super()
		는 생략할 수 있지만, 앞에 다른 문장이 먼저 기술되면 에러가
	...
	public void movigCal() {
		...
		받은 자식클래스에서도 접근할 수 없어 에러가 발생된다. 이 경우
		public으로 선언된 메서드를 통해 호출해야 한다. */
//		keyboardTouch();	//에러
//		calculate();		//발생
		calculateFunc();
...
/* 테블릿 노트북 : 컴퓨터의 기본 연산기능과 휴대이용 그리고 전용 펜으로
...
class TabletNotebook extends NotebookComputer{
	...
	public void write(String penInfo) {
		...
		기능처럼 사용할 수 있다. 단 접근이 가능한 지정자로 선언되어야 한다.
		super는 생략 할 수 있다.
		*/
		super.movigCal();
