...
오버라이딩(Overriding)
 ...
 완전히 동일한 모양으로 메서드를 재저의 하는 것을 말한다.
 오버로딩과 다른점은 매개변수의 갯수 및 타입, 반환타입 등이 완전히
 똑같은 형태로 정의하는 것이다.
 하위클래스에서 오버라이딩한 메서드는 상위클래스의 메서들를 가리게
...
class BaseSpeaker extends Speaker {
	...
	/* @Override : 어노테이셔이라 부르고, 오버라이딩된 메서드에
	 	추가하여 컴파일러에게 알려주는 역할을 한다. 필수사항은 아니므로
	 	생략할 수 있다. */
	@Override
	public void showState() {
