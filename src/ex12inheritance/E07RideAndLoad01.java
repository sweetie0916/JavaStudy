...
오버로딩 : 메서드명은 같지만 매개변수의 갯수, 타입, 순서가 다른 
	경우를 말한다. 즉, 서로 다른 메서드이므로 상속받은 하위클래스에서는
	오버로딩 된 모든 메서드가 포함된다.
...
public class E07RideAndLoad01 {
	...
	public static void main(String[] args) {
		...
		호출된다. 즉 아래 부분은 모두 C클래스에 정의된 rideMethod가
		호출된다. 오버라이딩은 참조변수의 영향을 받지 않는다. */
		...
		영역에 접근할 수 있다. 따라서 아래 3개의 문장은 모두 실행된다. */
		ref3.loadMethod();
		ref3.loadMethod(1);
		ref3.loadMethod(1.1);
...
//		C refId3 = refId1;
		...
		하다. 하지만 다운캐스팅(강제형변환)을 하면 참조가 가능해진다.
		즉, 부모타입을 자식타입으로 변경하는 것이다. 이는
		클래스가 상속관계에 있기 때문에 가능하고, 방식은 기본자료형과
		동일하게 소괄호를 이용하면 된다. */
		B refId2 = (B)refId1;
		C refId3 = (C)refId1;
	}
