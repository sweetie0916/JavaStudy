...
class Car {
	...
	표현하는 또 하나의 자료형을 생성하는것이 class를 작성하는 목적이 된다. */
	...
	/* 초기화메서드2 : 위와 동일한 이름의 메서드로 오버로딩에 의해 정의되어 있다.
	...
	//'운전하다' 기능을 구현한 멤버메서드
	void drive() {
		/* owner는 객체이므로 이름을 출력하기 위해 '변수명'.name'과 같이
		기술해야한다. */
...
public class E03CarMain {
	...
	public static void main(String[] args) {
		...
		*/
		...
		생성할때마다 동일한 코들르 작성해야 하므로 중복이 많아지게 
		...
		인스턴스를 생성한다. 이 경우 매개변수를 통해 다양한 값을 전달할 수 있으므로
		여러형태의 객체를 생성할 수 있다. 따라서 가장 효울적인 코드라 할 수 있다. 
		*/
		Car car3 = new Car();
		car3.initialize("스포츠카", "성유겸", 8, 10);
		car3.showCarInfo();

		/* 해당 인스턴스는 생성만하고 초기화하지 않은 상태로 
