...
public class Ex03ArrayList2 {
	...
	public static void main(String[] args) {
		...
		list2.add(st3);
		list2.add(st4); //인덱스 3
		//List는 중복저장이 가능핟. 인덱스로 구분할 수 있다.
		list2.add(st2); 
		
		System.out.println("[출력1-일반for문]");
		/* 일반 for문 사용을 위해 size()메서드로 추가된 인스턴스의
		갯수를 반환받는다. 또한 index로 접근해야하므로 get()메서드를 사용한다. */
		for(int i=0 ; i<list2.size() ; i++) {
			/* List의 각 인덱스에 저장된 인스턴스의 참조값을 그대로 출력한다. 
			toString() 메서드를 오버라이딩 했으므로 객체에 저장된 멤버변수값이
			출력된다. 만약 오버라이딩이 되어있지 않다면 참조값이 출력될 것이다. */
		...
		*/
		...
		for(Student st : list2) {
			...
			저장된 name과 일치하는지 확인한 후 해당 인스턴스를 찾을 수 있다. */
